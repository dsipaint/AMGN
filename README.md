...
## The webpanel
The purpose of AMGN is to simplify being a bot developer- if you are a bot developer there's a good chance you'd want to include a webpanel for users to configure your bot for their server or network. This is why AMGN comes pre-packaged with an auto-generated webpanel. The website is hosted on the address the bot is run on, on port 8080. You can see this by running the jar locally and visiting `localhost:8080/homepage.html`. There are a number of options you will see here.
In order to use the webpanel and be able to login to your network, the application needs to use Discord's OAuth2 methods. This means your network needs to know your application secret in order to log users into your app.
